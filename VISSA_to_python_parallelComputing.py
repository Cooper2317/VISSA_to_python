...
def vissa(Xcal, ycal, nLV_max=10, fold=5, method='center', num_bms=5000, ratio=0.05):
    ...
    for k in range(1, 101):  # 循环最多执行100次
        ...
        print('第{}轮 VISSA 已完成。耗时：{:.2f}s'.format(k,time.time()-vissa_time))
...
def vissa_round(Xcal, ycal, nLV_max=10, fold=5, method='center', num_bms=5000, ratio=0.05):
    ...
    for i in range(200):
        ...
        # 使用joblib进行并行计算所有子模型的RMSECV
        RMSECV_results = Parallel(n_jobs=-1, verbose=0)(
            delayed(compute_rmsecv)(j, Xcal, ycal, nLV_max, fold, method, binary_matrix) for j in range(num_bms)
        )
        RMSECV = np.array(RMSECV_results).reshape(1, -1)
...
def compute_rmsecv(j, Xcal, ycal, nLV_max, fold, method, binary_matrix):
    Xcal_subset = np.squeeze(Xcal[:, binary_matrix[j, :] == 1])
    CV_result = plscvfold(Xcal_subset, ycal, nLV_max, fold, method, 0, 0)
    return CV_result['RMSECV']
...
'''
运行结果：
第1次二进制矩阵采样已完成。耗时：6.07s
第2次二进制矩阵采样已完成。耗时：5.67s
第3次二进制矩阵采样已完成。耗时：5.99s
第4次二进制矩阵采样已完成。耗时：5.41s
第5次二进制矩阵采样已完成。耗时：5.26s
第6次二进制矩阵采样已完成。耗时：4.88s
第7次二进制矩阵采样已完成。耗时：4.81s
第8次二进制矩阵采样已完成。耗时：4.61s
第9次二进制矩阵采样已完成。耗时：4.51s
第10次二进制矩阵采样已完成。耗时：4.39s
第11次二进制矩阵采样已完成。耗时：4.39s
第12次二进制矩阵采样已完成。耗时：4.33s
第13次二进制矩阵采样已完成。耗时：4.27s
第1轮 VISSA 已完成。耗时：68.80s
第1次二进制矩阵采样已完成。耗时：3.01s
第2次二进制矩阵采样已完成。耗时：3.30s
第3次二进制矩阵采样已完成。耗时：3.62s
第4次二进制矩阵采样已完成。耗时：3.72s
第5次二进制矩阵采样已完成。耗时：3.78s
第6次二进制矩阵采样已完成。耗时：3.86s
第7次二进制矩阵采样已完成。耗时：3.73s
第8次二进制矩阵采样已完成。耗时：3.64s
第2轮 VISSA 已完成。耗时：32.50s
第1次二进制矩阵采样已完成。耗时：2.99s
第2次二进制矩阵采样已完成。耗时：3.39s
第3次二进制矩阵采样已完成。耗时：3.45s
第4次二进制矩阵采样已完成。耗时：3.57s
RMSEP: 0.7174209923198565, RMSEC: 0.5064618385274829
总耗时：1.98min
# RMSEP、RMSEC会有变动 #
'''